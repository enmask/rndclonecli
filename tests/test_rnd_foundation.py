...
def test_step_realtime_frame_routes_editor_save_action_and_sets_feedback(tmp_path) -> None:
    level_path = tmp_path / "editor-save.txt"
    state = parse_level(
        [
            "#####",
            "#P  #",
    ...
    assert level_path.read_text(encoding="utf-8") == "#####\n#Ps #\n#####\n"
    assert state.editor_file_feedback == "saved editor-save.txt + sidecar"
    assert state.editor_file_feedback_is_error is False
...
def test_main_loads_file_backed_level_for_graphics_runtime(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
) -> None:
    captured: dict[str, object] = {}
    level_path = tmp_path / "cli-level.txt"
    level_path.write_text("#####\n#Pmg#\n#####\n", encoding="utf-8")
    (tmp_path / "cli-level.elements.json").write_text(
        (
            '{\n'
            '  "format": "rndclonecli.level-elements",\n'
            '  "version": 1,\n'
    ...
    monkeypatch.setattr(
        "sys.argv",
        ["rnd_foundation.py", "--graphics2d", "--level", str(level_path)],
    )
    ...
    assert captured == {
        "level_path": str(level_path),
        "sidecar_path": str(tmp_path / "cli-level.elements.json"),
        "mud_cell": "mud",
        "gem_cell": "gem2",
        "diamonds_total": 1,
...
def test_main_loads_file_backed_level_for_turn_based_mode(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
) -> None:
    captured: dict[str, object] = {}
    level_path = tmp_path / "turn-level.txt"
    level_path.write_text("#####\n#P. #\n#####\n", encoding="utf-8")

    def fake_run_interactive_turn_based(state: GameState) -> None:
        captured["level_path"] = state.level_path
        captured["sand_cell"] = state.get_cell(2, 1)
...
def test_game_state_reports_open_cells_for_entry() -> None:
    ...
    assert state.is_open_for_entry(3, 1) is True
...
def test_update_graphics_frame_tracks_and_renders_horizontal_custom_slime_push_motion_when_enabled(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    install_fake_pygame(monkeypatch)
    registry = dict(DEFAULT_CUSTOM_ELEMENTS)
...
def test_update_graphics_frame_tracks_and_renders_horizontal_custom_slime_snap_push_motion_when_enabled(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    install_fake_pygame(monkeypatch)
    registry = dict(DEFAULT_CUSTOM_ELEMENTS)
...
def test_graphics_mode_editor_save_control_writes_level_and_sets_feedback(
    ...
    level_path = tmp_path / "graphics-save.txt"
    state = parse_level(
        [
            "#####",
            "#P  #",
